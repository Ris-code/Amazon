...
function updateChartbar(chartId, selectedYear) {
    ...
    d3.csv('static/customer_orders.csv').then(function(data) {
        ...
        // Group by month for spending and savings using d3.group on filteredData
        const groupedData = d3.group(filteredData, d => d['Order Month']);

        // Prepare arrays for labels (months) and data (monthly spending and savings)
        ...
        const monthlySavings = monthNumbers.map(monthNum => {
            const monthData = groupedData.get(monthNum) || [];
            return d3.sum(monthData, d => d['Savings']);
        });
